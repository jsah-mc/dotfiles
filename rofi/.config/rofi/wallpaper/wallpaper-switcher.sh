...
if [[ -r "$current_theme" ]] && [[ "$(cat "$current_theme")" == "matugen" ]]; then
    if [[ ! -x "$theme_script" ]]; then
        notify "Wallpaper" "Matugen theme script is missing: $theme_script"
        exit 1
    fi
    ...
    notify "Wallpaper" "Applied $(basename "$wallpaper") and regenerated matugen"
    exit 0
fi
